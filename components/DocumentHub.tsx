...
// SRE FIX: Added missing icons Minimize2 and Maximize2 to imports
import {
...
const DocumentHub = ({ systemInfo, currentUser }: DocumentHubProps) => {
    ...
    // SRE INIT: Carrega conteúdo no DOM apenas uma vez ao abrir o editor
    useEffect(() => {
    ...
    const updateStats = useCallback(() => {
        if (editorRef.current) {
            ...
            setShowPageWarning(currentHeight > (estPages * 1123 * 0.95));
        }
    }, []);
    ...
    const handleSave = async () => {
        ...
        try {
            // Sincroniza o conteúdo do DOM para o payload
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (editorRef.current && content) {
                if (editorRef.current.innerText.trim() === "" || editorRef.current.innerText.includes("Comece a escrever")) {
                    editorRef.current.innerHTML = content;
                } else {
                    execCommand('insertHTML', `<div style="margin-top:20px; border-top: 1px dashed #ccc; padding-top: 20px;">${content}</div>`);
                }
            }
            setAiPrompt('');
            updateStats();
        } catch (e) {
            alert("⚠️ Falha no Ghostwriter. Verifique a API_KEY.");
        } finally {
            ...
            <header className="flex flex-col md:flex-row justify-between items-center bg-slate-900 p-8 rounded-[2.5rem] shadow-xl text-white shrink-0 overflow-hidden relative">
                ...
                <div className="flex items-center gap-5 relative z-10">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-indigo-400 font-black uppercase tracking-widest mt-2 opacity-80">SRE Editor Sincronizado V190.0</p>
                    </div>
                </div>
                <div className="flex gap-4 relative z-10">
                    <button onClick={() => setShowOCR(true)} className="p-4 bg-white/5 hover:bg-white/10 text-white rounded-xl transition-all border border-white/5 shadow-xl group"><Camera size={20} /></button>
                    {canManage && (
                        <button onClick={() => handleOpenEditor(null)} className="px-10 py-4 bg-white text-slate-950 rounded-xl font-black text-[11px] uppercase tracking-widest shadow-2xl hover:bg-indigo-50 transition-all flex items-center gap-3 active:scale-95">
            ...
            <div className="bg-white rounded-[3rem] border border-slate-200 shadow-sm flex flex-col flex-1 overflow-hidden">
                <div className="p-8 border-b bg-slate-50/50 flex justify-between items-center shrink-0">
                    <div className="relative w-full max-w-md">
                        ...
                        <input type="text" placeholder="Filtrar base de protocolos..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-14 pr-6 h-14 bg-white border border-slate-200 rounded-2xl text-sm font-bold shadow-inner outline-none transition-all uppercase" />
                    </div>
                                ...
                                <div key={doc.id} className="bg-white p-8 rounded-[3rem] border border-slate-100 hover:border-indigo-200 hover:shadow-2xl transition-all group flex flex-col min-h-[300px]">
                                    <div className="flex justify-between items-start mb-8">
                                        ...
                                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0">
                                            {canManage && (
                                                <button onClick={(e) => { e.stopPropagation(); if (confirm("Excluir registro?")) documentService.delete(doc.id).then(loadDocuments); }} className="p-3 bg-white border border-slate-100 text-slate-300 hover:text-rose-600 rounded-xl shadow-sm"><Trash2 size={16} /></button>
                                            )}
            ...
            {isEditorOpen && activeDoc && (
                <div className="sie-editor-overlay">
                    <div className={`sie-modal-container ${isFullScreen ? '!w-screen !h-screen !rounded-none !max-w-none border-none' : ''}`}>
                            ...
                            <div className="flex items-center gap-8">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <div className="flex items-center gap-4 mt-1">
                                        <p className="text-indigo-400 text-[9px] font-black uppercase tracking-widest opacity-80">
                                            EDITOR DIRETO ATIVO • {stats.words} PALAVRAS
                                        </p>
                                        <span className="text-emerald-400 text-[9px] font-black uppercase tracking-widest bg-white/5 px-2 py-0.5 rounded">~{stats.pages} FOLHAS A4</span>
                                    </div>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <button onClick={() => window.print()} className="p-4 bg-white/5 hover:bg-emerald-600 hover:text-white text-white rounded-2xl border border-white/5 transition-all" title="Imprimir"><Printer size={20} /></button>
                                <button onClick={() => setIsFullScreen(!isFullScreen)} className="p-4 bg-white/5 hover:bg-indigo-600 hover:text-white text-white rounded-2xl border border-white/5 transition-all">
                                ...
                                <button onClick={handleSave} disabled={isSaving} className={`px-10 py-4 ${saveStatus === 'SUCCESS' ? 'bg-emerald-600' : 'bg-indigo-600 hover:bg-indigo-500'} text-white rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all flex items-center gap-3 shadow-xl active:scale-95`}>
                                    ...
                                    {saveStatus === 'SUCCESS' ? 'Sincronizado' : 'Commitar Protocolo'}
                                </button>
                        ...
                        {/* TOOLBAR */}
                        <div className="sie-editor-toolbar flex items-center bg-white border-b px-10 py-3 shrink-0 gap-8 z-40 shadow-sm overflow-x-auto custom-scrollbar">
                            <div className="flex items-center gap-1.5 bg-slate-50 p-1.5 rounded-2xl border border-slate-200">
                                <button onClick={() => execCommand('bold')} className="p-2.5 hover:bg-white hover:text-indigo-600 rounded-xl transition-all text-slate-500"><Bold size={18} /></button>
                                <button onClick={() => execCommand('italic')} className="p-2.5 hover:bg-white hover:text-indigo-600 rounded-xl transition-all text-slate-500"><Italic size={18} /></button>
                                <button onClick={() => execCommand('underline')} className="p-2.5 hover:bg-white hover:text-indigo-600 rounded-xl transition-all text-slate-500"><Underline size={18} /></button>
                            </div>
                            <div className="flex items-center gap-3">
                                <button onClick={() => execCommand('insertHTML', '<div class="page-break"></div><p><br/></p>')} className="px-5 py-3 bg-blue-50 border border-blue-100 rounded-2xl text-blue-700 text-[10px] font-black uppercase"><Scissors size={18} /> Quebra</button>
                            </div>
                        </div>

                        <div className="flex-1 flex overflow-hidden bg-slate-200 relative">
                            {!isFullScreen && (
                                <div className="sie-editor-sidebar hidden xl:flex w-[440px] bg-white border-r flex-col gap-0 shrink-0 shadow-2xl z-30">
                                    <div className="flex-1 flex flex-col p-10 gap-8 overflow-y-auto custom-scrollbar">
                                        <div className="flex items-center gap-6">
                                            <div className="p-5 bg-indigo-600 rounded-[1.5rem] text-white shadow-2xl animate-pulse"><Wand2 size={28} /></div>
                                            <div>
                                                <h5 className="text-[14px] font-black uppercase tracking-[0.2em] text-slate-900 leading-none">Ghostwriter</h5>
                                                <p className="text-[9px] font-bold text-slate-400 uppercase mt-2">Redação Neural Ativa</p>
                                            </div>
                                        </div>
                                        <div className="space-y-6">
                                            <textarea
                                                className="w-full bg-slate-50 border border-slate-200 rounded-[2.5rem] p-10 text-sm font-medium outline-none focus:bg-white focus:border-indigo-500 transition-all uppercase shadow-inner resize-none h-[250px]"
                                            ...
                                            <button
                                                ...
                                                className="w-full py-6 bg-slate-950 text-white rounded-[2.5rem] font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-indigo-600 transition-all flex items-center justify-center gap-3"
                                            >
                                                {isGenerating ? <Loader2 className="animate-spin" size={18} /> : <Sparkles size={18} />} Gerar Texto Neural
                                            </button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-12 md:p-20 flex flex-col items-center bg-slate-300/40 print:bg-white print:p-0">
                                <div
                                    ref={editorRef}
                                    contentEditable="true"
                                    suppressContentEditableWarning
                                    onInput={updateStats}
                                    className={`sie-printable-content w-full ${isFullScreen ? 'max-w-[950px]' : 'max-w-[850px]'} shadow-2xl transition-all duration-700 mx-auto bg-white min-h-[1123px] outline-none cursor-text`}
                                    spellCheck="false"
                                />
                                <div className="h-60 w-full shrink-0 print:hidden"></div>
                            </div>
                        </div>
                    </div>
                </div>
