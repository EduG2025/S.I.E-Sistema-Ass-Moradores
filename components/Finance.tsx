// --- START OF FILE Paste January 28, 2026 - 7:09PM ---

import React, { useState, useEffect, useMemo, useCallback } from 'react'; // <ADICAO ADITIVA> Hooks importados
...
import {
    ...
    AlertCircle, Search, Eye, DollarSign, Clock
} from 'lucide-react';
...
// <ADICAO ADITIVA> Tipagem auxiliar para acomodar campos de edição temporários
type EditableFinancialRecord = FinancialRecord & {
...
const Finance = ({ systemInfo }: FinanceProps) => {
    ...
    const [editingRecord, setEditingRecord] = useState<EditableFinancialRecord | null>(null);
    ...
    const handleSave = async (e: any) => {
        ...
        if (!editingRecord || !editingRecord.description || !editingRecord.amount) return alert("Dados incompletos.");
        setIsSaving(true);
        try {
            // LÓGICA ADITIVA DE MAPEAMENTO: Converte campos legados/temporários para o novo padrão 'recurrence'
            const payloadToSend: FinancialRecord = {
                ...editingRecord,
                // Se is_recurring for 1, define a recorrência. Caso contrário, é NONE.
                recurrence: editingRecord.is_recurring === 1 ? editingRecord.billing_cycle || 'MONTHLY' : 'NONE',
                status: editingRecord.status || 'PENDING',
                type: editingRecord.type,
                date: editingRecord.date,
                amount: Number(editingRecord.amount) // Garantindo que o amount seja número
            } as FinancialRecord;

            // Limpeza de campos temporários/legados antes de enviar
            delete payloadToSend.is_recurring;
            delete payloadToSend.billing_cycle;
            delete payloadToSend.next_due_date;

            if (editingRecord.id) await financialService.update(editingRecord.id, payloadToSend);
            else await financialService.create(payloadToSend);
    ...
    const handleExport = useCallback(async (type: 'CSV' | 'PDF') => {
        ...
        if (type === 'CSV') {
            const headers = ["Data Emissão", "Vencimento", "Descrição", "Categoria", "Tipo", "Valor", "Status"];
    ...
    }, [records, systemInfo.shortName]);

    const categoryData = useMemo(() => FINANCIAL_CATEGORIES.map(cat => ({
        name: cat,
        value: records.filter(r => r.category === cat).reduce((acc, r) => acc + Number(r.amount), 0)
    })).filter(c => c.value > 0), [records]);

    const COLORS = [systemInfo.primaryColor || '#4f46e5', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4'];

    return (
        <div className="flex h-full flex-col space-y-4 animate-fade-in relative overflow-hidden">
            <div className="flex flex-row justify-between items-center bg-slate-900 p-6 rounded-[2.5rem] text-white shadow-xl shrink-0 overflow-hidden relative">
                ...
                <div className="flex gap-3 relative z-10">
                    ...
                    {/* <ADICAO ADITIVA> Inicialização do estado de edição (garantindo que o objeto criado seja do tipo esperado, mesmo sem 'id') */}
                    <button onClick={() => {
                        setEditingRecord({
                            description: '',
                            amount: '',
                            ...
                            status: 'PENDING',
                            is_recurring: 0, // Mantido como 0/legado
                            recurrence: 'NONE' // Novo campo
                        } as EditableFinancialRecord); // <ADICAO ADITIVA> Casting explícito para aceitar objeto sem 'id'
                        setIsModalOpen(true);
                    }} className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-2xl flex items-center gap-3 transition-all" style={{ backgroundColor: systemInfo.primaryColor }}><Plus size={18} /> Novo Lançamento</button>
                </div>
            </div>
                    ...
                    {activeTab === 'REPORTS' ? (
                        ...
                        <table className="w-full text-left border-separate border-spacing-0">
                            <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b sticky top-0 z-10 backdrop-blur-md">
                                <tr className="bg-white/95"><th className="p-8 border-b">Protocolo / Emissão</th><th className="p-8 border-b text-center">Vencimento</th><th className="p-8 border-b">Modo / Classificação</th><th className="p-8 text-right border-b">Montante</th><th className="p-8 text-right border-b">Gestão</th></tr>
                            </thead>
                                    ...
                                    <tr key={r.id} className="hover:bg-slate-50 transition-colors group">
                                        ...
                                        <td className="p-8 text-center">
                                            <div className="flex flex-col items-center">
                                                <span className={`text-sm font-black ${r.status !== 'PAID' && new Date(r.next_due_date || '') < new Date() ? 'text-rose-600' : 'text-slate-700'}`}>
                                                    {r.next_due_date ? new Date(r.next_due_date).toLocaleDateString('pt-BR') : '---'}
                                                </span>
                                                {r.status !== 'PAID' && new Date(r.next_due_date || '') < new Date() && (
                                                    <span className="text-[8px] font-black text-rose-500 uppercase tracking-tighter mt-1 animate-pulse">Vencido</span>
                                                )}
                                            </div>
                                        </td>
                                        <td className="p-8">
                                            <div className="flex items-center gap-3">
                                                ...
                                                {(r as any).is_recurring ? <span title="Recorrência Ativa (Legado)" className="p-1.5 bg-amber-50 text-amber-600 rounded-lg border border-amber-100"><RefreshCw size={12} /></span> : null}
                                            </div>
                                            {r.recurrence === 'MONTHLY' && <span title="Recorrência Mensal" className="p-1.5 bg-amber-50 text-amber-600 rounded-lg border border-amber-100 text-[8px] font-bold uppercase mt-1 inline-block">Mensal</span>}
                                        </td>
                                        ...
                                        <td className="p-8 text-right"><button onClick={() => { setEditingRecord(r as EditableFinancialRecord); setIsModalOpen(true); }} className="p-4 text-slate-300 hover:text-indigo-600 hover:bg-white hover:shadow-xl rounded-2xl transition-all border border-transparent hover:border-indigo-100"><Edit2 size={18} /></button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                                ...
                                <div className="bg-slate-50 p-10 rounded-[3rem] border border-slate-200 shadow-inner space-y-10">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="space-y-2">
                                            ...
                                            <input type="date" required className="w-full font-black h-14 bg-white border border-slate-200 rounded-xl px-6 text-xl" value={editingRecord.date} onChange={e => setEditingRecord({ ...editingRecord, date: e.target.value })} />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-rose-500 uppercase tracking-widest ml-1">Data de Vencimento</label>
                                            <div className="relative">
                                                <Clock className="absolute right-4 top-1/2 -translate-y-1/2 text-rose-300" size={20} />
                                                <input type="date" required className="w-full font-black h-14 bg-white border border-rose-200 rounded-xl px-6 text-xl focus:border-rose-500 shadow-sm" value={editingRecord.next_due_date} onChange={e => setEditingRecord({ ...editingRecord, next_due_date: e.target.value })} />
                                            </div>
                                        </div>
                                    </div>
