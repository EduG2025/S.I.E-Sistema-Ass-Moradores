...
/**
 * S.I.E Resident Dashboard V6.3 - MEMBER CORE
 * SRE Operational Standard - Fully Dynamic Labels
 */
...
const ResidentDashboard = ({ onNavigate, systemInfo }: ResidentDashboardProps) => {
  ...
  // Governança de Metadados - Textos Dinâmicos
  const metadata = useMemo(() => systemInfo?.module_metadata?.['resident_dashboard'] || {
    greeting: "Olá",
    unit_label: "Unid.",
    badge_label: "Associado",
    key_btn: "Chave SRE",
    ouvidoria_btn: "Ouvidoria",
    balance_label: "Aberto",
    balance_cta: "Ver Faturas",
    agenda_label: "Agenda",
    agenda_cta: "Reservar Área",
    access_label: "Acesso",
    access_cta: "Gerar Convite",
    mural_title: "Mural S.I.E",
    advisor_title: "Dúvidas? IA Mentor.",
    advisor_slogan: "Suporte normativo e regimental ativo.",
    advisor_cta: "Falar com Advisor",
    audit_label: "Dados Auditados",
    audit_slogan: "Monitorado pelo Protocolo SRE V4.5"
  }, [systemInfo]);
    ...
    <div className="flex-1 flex flex-col space-y-8 animate-fade-in max-w-[1300px] mx-auto">
      ...
      <div className="bg-slate-900 rounded-[3rem] p-10 relative overflow-hidden shadow-2xl border border-white/5">
        ...
        <div className="relative z-10">
          <div className="px-5 py-2 bg-indigo-500/10 border border-indigo-500/20 rounded-full flex items-center gap-2 backdrop-blur-md w-fit mb-10">
              ...
              <span className="text-[10px] font-black text-indigo-300 uppercase tracking-widest">{systemInfo.shortName} • {metadata.badge_label || "Associado"}</span>
          </div>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
              <div>
                <h1 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tightest leading-none">
                  {metadata.greeting || "Olá"}, {currentUser?.name.split(' ')[0]}
                </h1>
                <p className="text-slate-400 font-medium text-lg mt-4 uppercase tracking-widest flex items-center gap-3">
                   <Fingerprint size={20} className="text-indigo-500"/> {metadata.unit_label || "Unid."} {currentUser?.unit || 'PENDENTE'}
                </p>
              </div>
              <div className="flex flex-wrap gap-3">
                <button onClick={() => setIsResetModalOpen(true)} className="px-6 py-3.5 bg-white/5 hover:bg-white/10 text-white rounded-xl border border-white/10 font-black text-[9px] uppercase tracking-widest flex items-center gap-3 transition-all">
                  <Lock size={14}/> {metadata.key_btn || "Chave SRE"}
                </button>
                {isModuleEnabled('suggestions') && (
                    <button onClick={() => onNavigate('suggestions')} className="px-6 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-black text-[9px] uppercase tracking-widest flex items-center gap-3 shadow-xl transition-all" style={{ backgroundColor: primaryColor }}>
                      <MessageSquare size={14}/> {metadata.ouvidoria_btn || "Ouvidoria"}
                    </button>
                )}
              </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isModuleEnabled('finance') && (
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-indigo-300 transition-all cursor-pointer" onClick={() => onNavigate('finance')}>
                ...
                <div>
                    <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">{metadata.balance_label || "Aberto"}</p>
                    <h3 className="text-3xl font-black text-slate-800 tracking-tight">R$ {Number(data?.pendingBalance || 0).toLocaleString('pt-BR')}</h3>
                    <div className="mt-4 flex items-center gap-2 text-[8px] font-black text-indigo-600 uppercase tracking-widest bg-indigo-50 px-2.5 py-1 rounded-lg w-fit">
                      {metadata.balance_cta || "Ver Faturas"}
                    </div>
                </div>
            </div>
        )}
        ...
        {isModuleEnabled('reservations') && (
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-indigo-300 transition-all cursor-pointer" onClick={() => onNavigate('reservations')}>
                ...
                <div>
                    <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">{metadata.agenda_label || "Agenda"}</p>
                    <h3 className="text-3xl font-black text-slate-800 tracking-tight">{data?.reservations?.length || 0} Registros</h3>
                    <div className="mt-4 flex items-center gap-2 text-[8px] font-black text-emerald-600 uppercase tracking-widest bg-emerald-50 px-2.5 py-1 rounded-lg w-fit">
                      {metadata.agenda_cta || "Reservar Área"}
                    </div>
                </div>
            </div>
        )}
        ...
        {isModuleEnabled('concierge') && (
            <div className="bg-slate-950 p-8 rounded-[2.5rem] text-white shadow-2xl flex flex-col justify-between group hover:scale-[1.01] transition-all cursor-pointer overflow-hidden relative" onClick={() => onNavigate('concierge')}>
                ...
                <div className="relative z-10">
                    <p className="text-[9px] font-black uppercase text-indigo-400 tracking-widest mb-1">{metadata.access_label || "Acesso"}</p>
                    <h3 className="text-3xl font-black text-white tracking-tight">QR Code</h3>
                    <div className="mt-4 flex items-center gap-2 text-[8px] font-black text-white uppercase tracking-widest bg-white/10 border border-white/10 px-2.5 py-1 rounded-lg w-fit">
                      {metadata.access_cta || "Gerar Convite"}
                    </div>
                </div>
            </div>
        )}
            ...
            <div className="lg:col-span-8 bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    <h4 className="text-xl font-black text-slate-800 uppercase tracking-tight flex items-center gap-3">
                        <Bell size={24} className="text-indigo-600" style={{ color: primaryColor }}/> {metadata.mural_title || "Mural S.I.E"}
                    </h4>
                </div>
        ...
        {isModuleEnabled('neural_chat') && (
            <div className="lg:col-span-4 bg-indigo-600 p-10 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden flex flex-col justify-between group cursor-pointer" onClick={() => onNavigate('neural_chat')} style={{ backgroundColor: primaryColor }}>
                ...
                <div className="relative z-10">
                    ...
                    <h4 className="text-3xl lg:text-4xl font-black uppercase tracking-tightest leading-tight">
                      {metadata.advisor_title || "Dúvidas? IA Mentor."}
                    </h4>
                    <p className="text-indigo-100 text-xs mt-4 font-medium leading-relaxed uppercase italic opacity-80">
                      {metadata.advisor_slogan || "Suporte normativo e regimental ativo."}
                    </p>
                </div>
                <button className="mt-8 w-full py-4 bg-white text-indigo-950 rounded-[1.5rem] font-black text-[9px] uppercase tracking-[0.2em] shadow-xl relative z-10">
                  {metadata.advisor_cta || "Falar com Advisor"}
                </button>
            </div>
        )}
      </div>
      ...
      <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-5">
              <div className="p-4 bg-white text-indigo-600 rounded-2xl shadow-sm" style={{ color: primaryColor }}><ShieldCheck size={24}/></div>
              <div>
                  <h4 className="text-lg font-black text-slate-800 uppercase tracking-tight leading-none">{metadata.audit_label || "Dados Auditados"}</h4>
                  <p className="text-[9px] text-slate-400 font-bold uppercase mt-1">{metadata.audit_slogan || "Monitorado pelo Protocolo SRE V4.5"}</p>
              </div>
          </div>
            ...
            <form onSubmit={handleUpdatePassword}>
              <div className="h-16 px-8 bg-slate-900 text-white flex justify-between items-center shrink-0 border-b border-white/5">
                <div className="flex items-center gap-4">
                  <div className="p-2.5 bg-indigo-600 rounded-xl shadow-xl" style={{ backgroundColor: primaryColor }}><Key size={18}/></div>
              ...
              <div className="p-8 space-y-6 bg-[#fdfdfe]">
                ...
                <button type="submit" className="w-full py-5 bg-slate-900 text-white rounded-[1.25rem] font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-indigo-600 active:scale-95" style={{ backgroundColor: primaryColor }}>Sincronizar Chave</button>
              </div>
            </form>
          </div>
        </div>
