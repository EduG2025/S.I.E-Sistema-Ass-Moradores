...
// Adicionando textos de endereço à governança
const SYSTEM_TEXTS = {
    // Endereço
    LBL_CEP: "CEP",
...
const Settings = ({ systemInfo, onUpdateSystemInfo }: SettingsProps) => {
    ...
    // Inicialização segura do estado local
    ...
    const [isSearchingCEP, setIsSearchingCEP] = useState(false); // Novo estado
    ...
    // Sincronização de Props com Estado Local
    useEffect(() => {
        if (systemInfo) {
    ...
    // Lógica ViaCEP para endereço da SEDE
    ...
    const handleSaveInfo = async () => {
        ...
        try {
            const payload = {
                ...localInfo,
                // Garantir que o campo address obsoleto não seja enviado se os campos atômicos existirem
                address: undefined,
    ...
    // Definição padrão dos módulos para fallback
                    ...
                    <div className="space-y-12 animate-fade-in pb-12 max-w-6xl mx-auto">
                        <div className="bg-white p-12 rounded-[3.5rem] border border-slate-200 shadow-sm space-y-12">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                ...
                                {/* Endereço Atômico da Sede (Substituindo localInfo.address) */}
                                <div className="md:col-span-2 space-y-8 pt-6">
                                    <h4 className="text-sm font-black text-slate-800 uppercase tracking-[0.2em] flex items-center gap-3"><MapPin size={20} className="text-indigo-600" /> {SYSTEM_TEXTS.TITLE_HEADQUARTERS_ADDRESS}</h4>
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                                        <div className="space-y-2">
                                            ...
                                            <div className="relative">
                                                <input
                                                    className="w-full font-black h-14 bg-white border border-slate-200 rounded-xl px-6 text-lg focus:border-indigo-500 outline-none transition-all"
                                                    value={localInfo.cep || ''}
                                                    onChange={e => setLocalInfo({ ...localInfo, cep: formatCEP(e.target.value) })}
                                                    onBlur={handleCEPBlur}
                                                    maxLength={9}
                                                    placeholder={SYSTEM_TEXTS.PLACEHOLDER_CEP}
                                                />
                                                {isSearchingCEP && <Loader2 className="absolute right-4 top-1/2 -translate-y-1/2 animate-spin text-indigo-500" size={20} />}
                                            </div>
                                        </div>
                                        <div className="space-y-2 md:col-span-3">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{SYSTEM_TEXTS.LBL_STREET}</label>
                                            <input className="w-full h-14 bg-slate-50 border border-slate-200 rounded-xl px-6 text-base uppercase outline-none" value={localInfo.street || ''} onChange={e => setLocalInfo({ ...localInfo, street: e.target.value })} />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{SYSTEM_TEXTS.LBL_NUMBER}</label>
                                            <input className="w-full h-14 bg-white border border-slate-200 rounded-xl px-6 text-base outline-none focus:border-indigo-500 transition-all" value={localInfo.number || ''} onChange={e => setLocalInfo({ ...localInfo, number: e.target.value })} placeholder={SYSTEM_TEXTS.PLACEHOLDER_NUMBER} />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{SYSTEM_TEXTS.LBL_COMPLEMENT}</label>
                                            <input className="w-full h-14 bg-white border border-slate-200 rounded-xl px-6 text-base outline-none focus:border-indigo-500 transition-all" value={localInfo.complement || ''} onChange={e => setLocalInfo({ ...localInfo, complement: e.target.value })} placeholder={SYSTEM_TEXTS.PLACEHOLDER_COMPLEMENT} />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{SYSTEM_TEXTS.LBL_NEIGHBORHOOD}</label>
                                            <input className="w-full h-14 bg-slate-50 border border-slate-200 rounded-xl px-6 text-base uppercase outline-none" value={localInfo.neighborhood || ''} onChange={e => setLocalInfo({ ...localInfo, neighborhood: e.target.value })} />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{SYSTEM_TEXTS.LBL_CITY_STATE}</label>
                                            <div className="flex gap-2">
                                                <input className="flex-1 h-14 bg-slate-50 border border-slate-200 rounded-xl px-6 font-bold uppercase outline-none focus:border-indigo-500 transition-all" value={localInfo.city || ''} onChange={e => setLocalInfo({ ...localInfo, city: e.target.value })} />
                                                <input className="w-16 h-14 bg-slate-50 border border-slate-200 rounded-xl text-center font-bold outline-none focus:border-indigo-500 transition-all" value={localInfo.state || ''} onChange={e => setLocalInfo({ ...localInfo, state: e.target.value })} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                {/* FIM DO ENDEREÇO ATÔMICO */}
                ...
                {activeTab === 'INTERFACE' && (
                    <div className="space-y-12 animate-fade-in pb-12 max-w-6xl mx-auto">
                        <div className="bg-white p-12 rounded-[3.5rem] border border-slate-200 shadow-sm space-y-12">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                { /* Renderização segura dos módulos com Optional Chaining */}
                                {((localInfo?.resident_ui_settings?.length ? localInfo.resident_ui_settings : defaultModules)).map((mod, idx) => (
                                    <div key={mod.id} className="p-8 bg-slate-50 border border-slate-200 rounded-[2.5rem] flex justify-between items-center group hover:bg-white transition-all shadow-sm">
                                        ...
                                        <button
                                            onClick={() => {
                                                // Handler seguro para toggle
                                                const currentSettings = localInfo?.resident_ui_settings?.length
                                                    ? [...localInfo.resident_ui_settings]
                                                    : [...defaultModules];

                                                if (currentSettings[idx]) {
                                                    currentSettings[idx].enabled = !currentSettings[idx].enabled;
                    ...
                    <div className="sie-modal-container !h-auto !max-w-md self-center">
                        ...
                        <div className="p-10 space-y-8 bg-[#fdfdfe]">
                            ...
                            <div className="pt-4 flex gap-4">
                                ...
                                <button onClick={handleSaveRole} disabled={isSaving} className="flex-1 py-5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-indigo-600 transition-all flex items-center justify-center gap-3">
