        ...
        <div className="flex-1 flex flex-col min-h-0 space-y-4 animate-fade-in h-full relative">
            <div className="flex flex-row justify-between items-center bg-slate-900 p-6 rounded-[2rem] text-white shadow-xl shrink-0 overflow-hidden relative">
                <div className="flex items-center gap-4 relative z-10">
                    <div className="p-3 bg-indigo-600 rounded-2xl shadow-xl" style={{ backgroundColor: primaryColor }}><Database size={20}/></div>
                    <div>
                        <h2 className="text-lg font-black tracking-tight uppercase leading-none">Censo & Inteligência</h2>
                        <p className="text-indigo-400 text-[7px] font-black uppercase tracking-widest mt-1 opacity-80">Full Social Architecture • Architecture V2</p>
                    </div>
                </div>
                <div className="flex gap-2 relative z-10">
                    <button onClick={handleInjectMaster} className="px-6 py-3 bg-white/5 hover:bg-white/10 text-emerald-400 rounded-xl font-black text-[8px] uppercase tracking-widest transition-all flex items-center gap-2 border border-white/5">
                        <HandHelping size={16} /> Injetar Mestre
                    </button>
                    <button onClick={() => { setEditingSurvey({ title: '', description: '', type: 'CENSUS', questions: [], status: 'ACTIVE' }); setIsModalOpen(true); }} className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-black text-[9px] uppercase tracking-widest shadow-2xl transition-all active:scale-95 flex items-center gap-2" style={{ backgroundColor: primaryColor }}>
                        <Plus size={16} /> Novo
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pb-6">
                {isLoading ? <div className="p-20 text-center"><Loader2 className="animate-spin text-indigo-600 mx-auto" size={32}/></div> : (
                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3">
                        {surveys.map(s => (
                            <div key={String(s.id)} className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm hover:shadow-lg transition-all group flex flex-col h-fit relative">
                                <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                    <button onClick={() => handleCopyLink(s.id)} className="p-2 bg-white border border-slate-100 text-slate-400 hover:text-indigo-600 rounded-xl shadow-sm transition-all"><Share2 size={12} /></button>
                                    <button onClick={() => { setEditingSurvey(s); setIsModalOpen(true); }} className="p-2 bg-white border border-slate-100 text-slate-400 hover:text-indigo-600 rounded-xl shadow-sm transition-all"><Edit2 size={12} /></button>
                                </div>
                                <div className="flex items-center gap-2 mb-4">
                                    <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-[6px] font-black uppercase rounded-lg border border-indigo-100">{s.type}</span>
                                    <span className={`px-2 py-0.5 ${s.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'} text-[6px] font-black uppercase rounded-lg border`}>{s.status}</span>
                                </div>
                    ...
                    <div className="sie-modal-container">
                        <div className="h-16 px-8 bg-slate-900 text-white flex justify-between items-center shrink-0 border-b border-white/10">
                            <div className="flex items-center gap-4">
                                <div className="p-2 bg-indigo-600 rounded-xl shadow-xl" style={{ backgroundColor: primaryColor }}><Brain size={18}/></div>
                                <div>
                                    <h3 className="font-black text-lg uppercase tracking-tight leading-none">Arquiteto de Senso</h3>
                                    <p className="text-indigo-400 text-[7px] font-black uppercase mt-1 tracking-widest opacity-80">Full Social Architecture • V2 UI</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                <button onClick={handleSuggestIA} disabled={isGeneratingIA} className="px-4 py-2 bg-white/5 hover:bg-white/10 text-indigo-300 rounded-xl font-black text-[8px] uppercase tracking-widest flex items-center gap-2">
                                    {isGeneratingIA ? <Loader2 className="animate-spin" size={12}/> : <Sparkles size={12}/>} IA Predict
                                </button>
                                <button onClick={handleSave} disabled={isSaving} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-black text-[9px] uppercase tracking-widest shadow-xl flex items-center gap-2" style={{ backgroundColor: primaryColor }}>
                                    {isSaving ? <Loader2 className="animate-spin" size={14}/> : <Save size={14}/>} Commitar
                                </button>
                                <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-rose-500 hover:text-white text-slate-400 rounded-xl transition-all"><X size={20}/></button>
                            </div>
                        </div>
                            ...
                            <div className="w-full space-y-8">
                                <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-200 space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                                        <div className="md:col-span-8 space-y-1">
                                            ...
                                            <input className="w-full font-black h-11 bg-white border border-slate-200 rounded-xl px-4 text-sm focus:border-indigo-500 outline-none uppercase" placeholder="Título..." value={editingSurvey.title} onChange={e => setEditingSurvey({ ...editingSurvey, title: e.target.value })} />
                                        </div>
                                        <div className="md:col-span-4 space-y-1">
                                            <label className="text-[7px] font-black text-slate-400 uppercase tracking-widest ml-1">Análise</label>
                                            <select className="w-full font-black h-11 bg-white border border-slate-200 rounded-xl px-4 text-[9px] uppercase shadow-sm outline-none focus:border-indigo-500 appearance-none" value={editingSurvey.type} onChange={e => setEditingSurvey({ ...editingSurvey, type: e.target.value })}>
                                ...
                                <section className="space-y-4">
                                    <div className="flex items-center gap-4 px-2">
                                        ...
                                        <span className="bg-emerald-50 text-emerald-600 text-[8px] font-black px-3 py-0.5 rounded-full border border-emerald-100">{editingSurvey.questions?.length || 0} Campos</span>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {editingSurvey.questions?.map((q: any, qIdx: number) => (
                                            <div key={q.id || qIdx} className="p-6 bg-white rounded-[2rem] border border-slate-200 relative group hover:border-indigo-300 transition-all">
                                                <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                                    <button onClick={() => {
                                                        const qs = [...editingSurvey.questions]; qs.splice(qIdx, 1); setEditingSurvey({...editingSurvey, questions: qs});
                                                    }} className="p-2 bg-rose-50 text-rose-400 rounded-xl hover:bg-rose-500 hover:text-white transition-all"><Trash2 size={12} /></button>
                                                </div>
                                                
                                                <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                                                    <div className="md:col-span-12 space-y-1">
                                                        <label className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Pergunta do Censo</label>
                                                        <input className="w-full font-black h-9 bg-slate-50 border border-slate-200 rounded-xl px-4 text-[9px] uppercase outline-none" value={q.text} onChange={e => updateQuestion(qIdx, 'text', e.target.value)} />
                                                    </div>
                                                    <div className="md:col-span-6 space-y-1">
                                                        <label className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Formato</label>
                                                        <select className="w-full font-black h-9 bg-slate-50 border border-slate-200 rounded-xl px-4 text-[8px] uppercase outline-none" value={q.type} onChange={e => updateQuestion(qIdx, 'type', e.target.value)}>
                                                    ...
                                                    <div className="md:col-span-6 space-y-1">
                                                        ...
                                                        <select className="w-full font-black h-9 bg-indigo-50 border-indigo-100 rounded-xl px-4 text-[8px] uppercase text-indigo-600 outline-none" value={q.mapping_tag} onChange={e => updateQuestion(qIdx, 'mapping_tag', e.target.value)}>
                        ...
                        <div className="p-6 border-t bg-slate-50 flex justify-between items-center shrink-0">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button onClick={handleSave} disabled={isSaving} className="px-8 py-2 bg-slate-900 text-white rounded-xl font-black text-[9px] uppercase tracking-widest shadow-xl">Salvar Estrutura</button>
                            </div>
                        </div>
                    </div>
                </div>
