    ...
    const loadUserData = async (userId: string | number) => {
        ...
        try {
            ...
            const [finRes, scoreRes] = await Promise.all([
                ...
                axios.get(`/api/users/${userId}/score`, { headers: { Authorization: `Bearer ${token}` } })
            ]);
        ...
        <div className="flex-1 flex flex-col min-h-0 space-y-6 animate-fade-in overflow-hidden">
            ...
            {editingUser && (
                <div className="fixed inset-0 flex items-center justify-center bg-slate-900/95 z-[9999] p-0 lg:p-6 backdrop-blur-xl animate-fade-in">
                    <div className="bg-white rounded-none lg:rounded-[3.5rem] shadow-2xl w-full max-w-7xl h-full lg:h-[92vh] overflow-hidden border border-white/10 flex flex-col animate-scale-in">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 lg:p-16 custom-scrollbar bg-[#fcfcfd]">
                            <div className="max-w-5xl mx-auto">
                                ...
                                {activeTab === 'AI_DOSSIER' && (
                                    <div className="bg-slate-900 p-16 rounded-[4rem] text-white text-center flex flex-col items-center justify-center relative overflow-hidden shadow-2xl border border-white/5 h-[500px]">
                                        ...
                                        <Brain size={80} className="mb-10 text-indigo-400 animate-pulse" />
                                        <h4 className="text-4xl font-black uppercase tracking-tightest leading-none">Auditoria Neural Pronta</h4>
                                        <p className="text-slate-400 text-lg mt-6 max-w-lg font-medium leading-relaxed">A IA cruzará telemetria financeira, participativa e social para gerar o dossiê preditivo de risco deste membro.</p>
                                        <button className="mt-12 px-16 py-6 bg-indigo-600 rounded-[2rem] font-black uppercase text-xs tracking-[0.4em] hover:bg-white hover:text-indigo-600 transition-all shadow-2xl active:scale-95 flex items-center gap-4"><Sparkles size={20}/> Iniciar Análise Ativa</button>
                                    </div>
                                )}
                            </div>
                        </div>
